...
- [ ] 修复了预测位置基准错误
- [x] 时间基准统一使用 fit_start_time_sec（速度函数的时间零点），避免了与消息时间戳混淆。
- [x] 优化初始飞行时间估算：改用 R 标中心距离，减少初值抖动，加速迭代收敛。
- [x] 拟合准确性有待进一步验证（当前使用 Ceres 带约束正弦拟合，稳定性较好）。
- [ ] odom 系坐标转换精度需结合 TF 树及相机标定进行实测评估,目前感觉问题有点大。
- [ ] 代码结构可进一步优化，但已具备完整功能，计划之后把这些全放到buff里,再和装甲板自瞄做模式转换。
