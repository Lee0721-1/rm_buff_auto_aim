...
public:
    ...
    : Node("buff_predictor_node", options)
    {
        ...
        small_omega_filter_ = std::make_unique<MedianFilter>(std::max(3, velocity_median_window_));
...
private:
 ...
 bool transformPointToTarget(
    ...
    void targetCallback(const buff_interfaces::msg::BuffTarget::SharedPtr msg)
    {
        ...
        if (depth > 0.0) {
            ...
            RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 1000,
                                "Invalid radius for depth solve: %.4f", msg->radius);
        }
        ...
        const float continuous_angle = angle_observer_->update(vector_x, vector_y, pixel_arm_length);
        ...
        if (is_big_buff) {
            ...
            time_since_start_ = now - fit_start_time_;
            if (!predictor_->has_fit_attempted() && time_since_start_ > 0 && time_since_start_ < fit_window_sec_) {
                ...
                float mean_angleVelocity =
                    (continuous_angle - last_angle_) / (time_since_start_ - last_time_since_start_);
                float filtered_angle_velocity = velocity_median_filter_->update(mean_angleVelocity);
                predictor_->time_w_pairs_.push_back({median_time, std::abs(filtered_angle_velocity)});
                if (debug_mode_) {
            ...
            if (!predictor_->is_completed() && time_since_start_ >= fit_window_sec_) {
                ...
                if (debug_mode_) {
                    const auto& p = predictor_->get_velocity_fit_params();
                    debug_csv_ << '\n' << p[0] << "," << p[1] << "," << p[2] << "," << p[3] << '\n' << std::endl;
                }
            }
            ...
            if (!small_first_) {
                double dt = now - small_last_time_;
                if (dt > 0.001) {
                    double raw_omega = (continuous_angle - small_last_angle_) / dt;
                    double filtered_omega = small_omega_filter_->update(raw_omega);
                    small_omega_ = filtered_omega;
                }
            } else {
                small_first_ = false;
            }
            small_last_angle_ = continuous_angle;
            small_last_time_ = now;

            // 将角速度填入消息（暂时复用 sin_omega 字段）
            aiming_msg.sin_omega = static_cast<float>(small_omega_);
            aiming_msg.sin_a = 0.0f;
            aiming_msg.sin_phi = 0.0f;
            aiming_msg.sin_b = 0.0f;
    ...
    // ========== 小符匀速估计相关成员 ==========
    double small_omega_ = 0.0;                  // 当前估计角速度 (rad/s)
    double small_last_angle_ = 0.0;             // 上一帧连续角度
    double small_last_time_ = 0.0;              // 上一帧时间戳
    bool small_first_ = true;
    std::unique_ptr<MedianFilter> small_omega_filter_; // 角速度中值滤波
};
